...
> 标有 <span class="faq-new">新增</span> 的问题为 2026 年 8 月 24 日补充说明。
...
### [爬虫] <span class="faq-new">新增</span> 歌词不完整可以吗？

可以。如果原网站本身只提供部分歌词，可以按照网站实际提供的内容保存。少量歌曲存在这种情况不会影响要求，但歌词不能大量缺失或为空。
...
### [爬虫] <span class="faq-new">新增</span> 需要登录才能访问的 URL 可以吗？

可以。只要该 URL 能够指向对应的歌曲或歌手页面即可。
...
### [爬虫] <span class="faq-new">新增</span> 可以通过 API 获取数据吗？

可以，但网页抓取和 HTML 解析应是主要的数据获取方式。API 可以用于补充部分字段，或在页面动态加载、访问受限等情况下辅助获取数据。

程序中需要包含能够实际运行、并用于生成最终数据的 HTML 抓取和解析逻辑。使用 API 时，仍需自己完成接口请求、返回数据解析、字段处理和保存，并在实验报告中说明使用 API 的原因。


### [爬虫] <span class="faq-new">新增</span> 验收时需要重新运行爬虫吗？

需要演示爬虫，但不要求重新爬取全部数据，运行少量样例即可。Cookie 可以手动配置；若目标网站临时无法访问，可使用已保存的 HTML 或 JSON 演示解析。

请勿将个人 Cookie、账号或 Token 提交到 Git 仓库。


### [爬虫] <span class="faq-new">新增</span> 可以使用第三方库获取加密歌词吗？

可以，使用第三方库本身不会导致扣分。需要在代码或实验报告中注明库的名称和来源，并能够解释这个库解决了什么问题、怎样调用，以及返回的数据如何被后续代码处理。

如果第三方库只负责歌词解密或格式转换，歌曲信息获取、接口调用、结果解析和数据保存仍由自己的程序完成，这种使用方式没有问题。

如果整个爬虫只剩下把歌曲 URL 传给第三方接口，再直接保存其返回结果，自己没有完成实质性的请求、解析或数据处理工作，验收时会根据实际完成的代码评分，相关部分不一定能得到满分。
